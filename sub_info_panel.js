...
[Script]
Sub_info = type=generic,timeout=10,script-path= https://raw.githubusercontent.com/tcqgg2018/surge/main/sub_info_panel.js,script-update-interval=0,argument=url=[URL encode 后的机场节点链接]&reset_day=1&title=AmyInfo&icon=bonjour&color=#007aff
...
(async () => {
  ...
  let content = [`使用：${bytesToSize(used)} | 总量：${bytesToSize(total)}/n` + `剩余：${bytesToSize(total-used)}`];
  if (resetDayLeft) {
